...
function exportToCSV(filename, rows) {
  const processRow = function (row) {
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      let result = innerValue.split('"').join('""');
...
const CoralIcon = ({ className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
    <path d="M22 22H2" /><path d="M12 22v-8" /><path d="M12 16c-2-2-3-2-5-4-1-1-2-1-2-3a2 2 0 0 1 4 0c0 1 1 2 3 3M12 14c2-1 3-2 5-4 1-1 2-2 2-4a2 2 0 0 0-4 0c0 1-1 2-3 4" /><path d="M7 22v-3" /><path d="M7 19c-1-1-2-1-3-2a1 1 0 0 1 2-1c1 0 1 1 2 2" /><path d="M17 22v-4" /><path d="M17 18c1-1 2-1 3-2a1 1 0 0 0-2-1c-1 0-1 1-2 2" />
  </svg>
);
...
const CoralReefIcon = ({ className }) => (
  <svg viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" className={className}>
    <g transform="translate(30, 20) scale(1.6)"><path d="M12 22v-8M12 16c-2-2-3-2-5-4-1-1-2-1-2-3a2 2 0 0 1 4 0c0 1 1 2 3 3M12 14c2-1 3-2 5-4 1-1 2-2 2-4a2 2 0 0 0-4 0c0 1-1 2-3 4M7 22v-3M7 19c-1-1-2-1-3-2a1 1 0 0 1 2-1c1 0 1 1 2 2M17 22v-4M17 18c1-1 2-1 3-2a1 1 0 0 0-2-1c-1 0-1 1-2 2" /></g>
    <g transform="translate(0, 45) scale(0.8) rotate(-10)"><path d="M10 22 Q 10 10 25 5 M10 22 Q 5 12 0 8 M10 22 Q 15 15 20 12" strokeWidth="1.5" /><circle cx="25" cy="5" r="1.5" fill="currentColor" opacity="0.5" /><circle cx="0" cy="8" r="1.5" fill="currentColor" opacity="0.5" /><circle cx="20" cy="12" r="1.5" fill="currentColor" opacity="0.5" /></g>
    <g transform="translate(65, 55) scale(0.7) rotate(15)"><path d="M12 22v-10M12 15 L5 8 M12 12 L20 5 M12 18 L18 15" strokeWidth="2" /></g>
    <circle cx="15" cy="85" r="2" fill="currentColor" opacity="0.3" /><circle cx="85" cy="75" r="1.5" fill="currentColor" opacity="0.2" /><circle cx="50" cy="90" r="1" fill="currentColor" opacity="0.4" /><path d="M5 95 Q 50 90 95 95" strokeWidth="1" opacity="0.3" />
  </svg>
);
...
function AdminLoginModal({ onVerify, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    onVerify(code, (success) => {
      ...
      if (!success) { setError(true); setCode(''); }
    });
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 z-[100] flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in">
      <div className="bg-white rounded-3xl w-full max-w-sm p-8 shadow-2xl relative animate-in zoom-in-95">
        <div className="flex justify-center mb-6"><div className="bg-blue-600 p-4 rounded-2xl text-white shadow-lg shadow-blue-200"><Lock className="w-8 h-8" /></div></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <input autoFocus type="password" value={code} onChange={e => { setCode(e.target.value); setError(false); }} placeholder="權限碼" className={`w-full p-4 bg-slate-50 border-2 rounded-2xl text-center text-2xl font-black tracking-[1em] outline-none transition-all ${error ? 'border-red-500 bg-red-50' : 'border-slate-100 focus:border-blue-500 focus:bg-white'}`} />
          ...
          <button type="button" onClick={onClose} disabled={isSubmitting} className="w-full py-2 text-slate-400 text-sm font-bold hover:text-slate-600 transition-colors disabled:opacity-50">取消返回</button>
        </form>
      </div>
    </div>
  );
...
function QuickCard({ icon, title, desc, onClick, colorTheme = "cyan", bgIcon }) {
  ...
  return (
    <div onClick={onClick} className={`bg-white/90 backdrop-blur-xl p-8 rounded-[2rem] border transition-all duration-500 cursor-pointer group hover:-translate-y-2 relative overflow-hidden ${theme.wrapper}`}>
      ...
      <div className={`absolute -bottom-10 -right-10 opacity-[0.05] group-hover:scale-110 group-hover:-translate-y-2 transition-all duration-700 pointer-events-none [&>svg]:w-48 [&>svg]:h-48 rotate-12 ${theme.watermark}`}>{bgIcon || icon}</div>
...
function BirthdaySelect({ label, required, value, onChange, dark = false }) {
  ...
  const getDaysInMonth = (y, m) => { if (!y || !m) return 31; return new Date(y, m, 0).getDate(); };
...
function BookingCard({ booking: b, type, db, appId }) {
  ...
  const executeDelete = async (e) => {
    e.stopPropagation();
    try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id)); } catch (e) { console.error(e); }
  };
         ...
         <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
               ...
               {confirmDelete ? (
                 ...
                 <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(true); }} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="永久刪除此紀錄"><Trash2 className="w-4 h-4" /></button>
               )}
            </div>
...
function BookingAdminPanel({ db, appId, bookings, type, title }) {
  ...
  const handleExport = () => {
    ...
    exportToCSV(`${title}_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.csv`, [headers, ...rows]);
  };

  return (
     <div className="h-full flex flex-col animate-in fade-in">
       <div className="p-6 border-b bg-slate-50 shrink-0 rounded-t-2xl flex justify-between items-center">
         <div><h3 className="text-xl font-bold text-slate-800">{String(title)}</h3><p className="text-slate-500 text-sm mt-1">處理顧客提交之預約單</p></div>
         {typeBookings.length > 0 && <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-bold shadow-sm hover:bg-green-700 transition-colors"><Download className="w-4 h-4" /> 匯出 EXCEL</button>}
       </div>
    ...
    <div className="animate-in fade-in duration-300">
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map(item => {
            let totalSlots = parseInt(item.capacity) || 0;
            let bookedCount = type === 'activity' ? bookings.filter(b => b.type === 'activity' && b.activityId === item.id && b.status !== 'cancelled').length : 0;
            let remainingSlots = Math.max(0, totalSlots - bookedCount);
            let isFull = remainingSlots === 0;
...
function RegistrationForm({ activity, onClose, onSubmit, sysConfig, onSuccess, equipments = [] }) {
  ...
  const isStepSuccess = step > totalSteps; 
  ...
  const handleSubmit = async () => {
    ...
    if (!isMedicalComplete) { setErrorMsg("⚠️ 請完成所有展開的醫療聲明選項 (包含各項子題目)！"); return; }
    if(isSubmitting) return;
    setIsSubmitting(true);
    ...
    try {
        ...
        if (medicalAnswers[q.id] === true) {
           ...
           if (q.subItems && q.subItems.length > 0) {
              q.subItems.forEach(sub => {
                 if (medicalAnswers[sub.id] === true) medicalIssues.push("↳ " + sub.text);
              });
           }
        }
      });
      ...
      const submitData = {
        ...
        name: f.name || '', nickname: f.nickname || '', phone: f.phone || '', idNumber: f.idNumber || '',
        birthday: f.birthday || '', height: f.height || '', weight: f.weight || '', shoeSize: f.shoeSize || '',
        weights: weights || { w1: 0, w2: 0, w25: 0, w3: 0 },
        rentals: rentals || [],
        selectedElectives: finalElectives || [],
        certFee: activity.certFee || 0, certSystem: activity.certSystem || '',
        useLocalShopEq: !!useLocalShopEq, isReturningCustomer: !!isReturningCustomer,
        accOption: isTrip ? 'trip' : (accOption || 'self'),
        divingExperience: exp || {}, medicalAnswers: medicalAnswers || {},
        medicalIssues: medicalIssues || [], hasMedicalIssue: medicalIssues.length > 0,
      ...
      if (isCourse && accOption === 'upgrade') {
        gotoAcc = true; accContext = { type: 'course_upgrade', date: activity.date, days: activity.days || 3, baseDeduct: 800 }; 
      } else if (!isCourse && !isTrip && accOption !== 'self') {
        gotoAcc = true; accContext = { type: 'activity_discount', date: activity.date, discountType: sysConfig.accDiscountType || 'fixed', discountVal: sysConfig.accDiscountValue || 0 };
      }

      setSubmitSuccessResult({ gotoAcc, accContext });
      setIsSubmitting(false);
        ...
        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1 bg-slate-50">
          {isStepSuccess && (
             <div className="flex flex-col items-center justify-center py-10 text-center animate-in zoom-in-95 duration-500">
               ...
               <h3 className="text-3xl font-black text-slate-800 mb-4">報名已成功送出！</h3>
               <p className="text-slate-500 font-bold leading-relaxed mb-8 max-w-md">
                 我們已經收到您的報名與裝備需求。<br/>
                 稍後可至「我的預約查詢」追蹤教練的審核進度。
               </p>
               {submitSuccessResult?.gotoAcc ? (
                 <div className="space-y-4 w-full max-w-sm">
                    <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl text-amber-800 text-sm font-bold shadow-sm mb-6">
                      🎉 系統偵測到您符合【專屬住宿優惠】資格！<br/>現在就去挑選您的舒適房間吧！
                    </div>
                    <button onClick={() => onSuccess(submitSuccessResult)} className="w-full py-4 bg-teal-600 text-white rounded-xl font-black shadow-lg shadow-teal-200 hover:bg-teal-700 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                      <Home className="w-5 h-5" /> 前往挑選住宿 (享優惠)
                    </button>
                    <button onClick={() => onSuccess({ gotoAcc: false })} className="w-full py-3 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-colors">
                      稍後再說，先查看紀錄
                    </button>
                 </div>
               ) : (
                 <button onClick={() => onSuccess({ gotoAcc: false })} className="w-full max-w-sm py-4 bg-blue-600 text-white rounded-xl font-black shadow-lg shadow-blue-200 hover:bg-blue-700 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                    <Search className="w-5 h-5" /> 查看我的預約紀錄
                 </button>
               )}
             </div>
          )}
          ...
          {!isStepSuccess && (
            <div className="animate-in slide-in-from-right-4 fade-in duration-300">
               <h3 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-2">
                  <div className="w-2 h-6 bg-blue-600 rounded-full"></div> {stepTitles[step-1].sub}
               </h3>
               {isStepOverview && (
                 <div className="space-y-6">
                   {activity.schedule && activity.schedule.some(s => {
                      ...
                      <div className="bg-indigo-50 border border-indigo-100 p-5 rounded-2xl shadow-sm">
                         <h4 className="text-sm font-black text-indigo-900 mb-4 flex items-center gap-2"><BookOpen className="w-5 h-5"/> 課程日程與內容安排</h4>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm h-full">
                         <h4 className="font-black text-slate-800 mb-4 border-b border-slate-100 pb-3 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-teal-500"/> 課程提供服務</h4>
                      ...
                      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm h-full">
                         <h4 className="font-black text-slate-800 mb-4 border-b border-slate-100 pb-3 flex items-center gap-2"><Plus className="w-5 h-5 text-purple-500"/> 必修、選修與加購資訊</h4>
                         <div className="space-y-4">
                            <div>
                   ...
                   {activity.notes && (
                      <div className="bg-amber-50 p-5 rounded-2xl border border-amber-200 shadow-sm">
                         <h4 className="font-black text-amber-900 mb-3 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-amber-500"/> 備註與注意事項</h4>
                         <p className="text-sm font-bold text-amber-800 whitespace-pre-wrap leading-relaxed">{activity.notes}</p>
                      </div>
                   )}
                 </div>
               )}
               ...
               {isStepEq && (
                 <div className="space-y-6">
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm animate-in slide-in-from-bottom-4">
                      <h3 className="font-black text-lg text-slate-800 mb-4 border-b border-slate-100 pb-2 flex items-center gap-2"><Scale className="w-5 h-5 text-blue-500" /> 配重需求與體型分析</h3>
                      {f.height && f.weight ? (
                        <div className="mb-6"><AISizeAdvisor height={f.height} weight={f.weight} shoeSize={f.shoeSize} showWeight={true} /></div>
                      ) : (
                        <div className="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-500 text-center">請回上一步填寫身高與體重，以啟用 AI 體型分析與裝備尺寸建議。</div>
                      )}
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 mt-10 bg-white rounded-[2rem] border border-slate-200 shadow-sm text-center animate-in fade-in zoom-in-95">
      ...
      <p className="text-slate-500 mb-8 font-bold leading-relaxed">
        專屬的住宿預訂與房控系統目前正在準備中，稍後將開放使用。<br/>
        若您已成功報名潛水活動，我們會保留您的優先升級權益。
      </p>
